...
type PlanDefinitionType =
    ...
    member this.ToIdentifierSystem() = patFhirUrl (this.ToCode())
...
type QuestionSectionType =
    ...
    member this.ToActivityDefinitionIdentifier(clinicUUID: Guid) =
        let system =
            patFhirUrl (sprintf "activityDefinitions/%s" (this.ToCoding().Code))

        Identifier(System = system, Value = clinicUUID.ToString())



type QuestionAnswerQuantities =
    | Kilograms
    | Metres
    | Steps

type QuestionAnswerTimePeriods =
    | Years
    | Months
    | Days
    | Hours
...
type QuestionAnswerType =
    ...
    static member TryParseDslType (strtype: string) (subtype: string) (selectionUuid: Guid option) =
        ...
        | "YES_NO_UNSURE"
        | "YES_NO" -> { Type = YesNo; Required = false } |> Some
    ...
    static member ParseDslType strType strSubtype selectionUuid (questionUuid: Guid) =
        ...
        | Some s -> s
        | None ->
            failwithf "unrecognised QuestionAnswerType: %s (subtype %s, question %A)" strType strSubtype questionUuid
    ...
    member this.toFhirValueSetReference() =
        ...
        let makeRef ident =
            referenceToBundleIdentifier (makeStaticIdentifier ident)
            |> Some

        let yesNo = ValueSets.identifiers.YesNo |> makeRef

        let yesNoUnsure =
            ValueSets.identifiers.YesNoUnsure |> makeRef

        let yesNoNAUnsure =
            ValueSets.identifiers.YesNoNaUnsure |> makeRef
        ...
        | Selection uuid ->
            let ident =
                Identifier(System = KnownUrls.patValueSetIdentifierSystem, Value = uuid.ToString())
...
type Answer =
    ...
    static member Parse(j: JsonValue) = Answer.Parse(j.AsString())
...
let makeQuestionCode (guid: Guid) summaryText =
    Coding(
        ...
        Display =
            (if (String.IsNullOrEmpty(summaryText) = false) then
                 summaryText
             else
                 null)
    )
