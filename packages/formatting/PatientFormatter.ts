...
export class PatientFormatter {
    constructor(public patient: FHIR.Patient) {}
     ...
     */
    get name() {
        function display(name: FHIR.HumanName) {
            const useText = false;
        ...
        function getOne() {
            const candidates = [names.find((n) => n.use === "official"), names.find((n) => n.use === "usual"), names[0]];
    ...
    private getAgeFullYears(dobOrString: Date | string) {
        const dob = typeof dobOrString == "string" ? new Date(dobOrString) : dobOrString;
    ...
    get medicareNumber() {
        ...
        const medicares = identifiers.filter((i) => i.system === "http://ns.electronichealth.net.au/id/hi/mc");
        if (medicares.length > 1) {
            throw new Error(`multiple medicare identifiers for patient: ${this.patient.id}`);
        }
    ...
    get addresses() {
        ...
        return address.map((a) => [(a.line || []).join(" "), a.city, a.postalCode].join(", ")).join("; ");
    }
    ...
    get ageGenderBornPhoneAddressMedicare() {
        ...
        return [this.ageGenderBorn, this.contactMobilePhone, this.contactHomePhone, this.addresses, medicare2]
            .filter((s) => s)
            .join(" | ");
    }
}
