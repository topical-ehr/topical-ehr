...
import { Interpretation, interpret, lookupReferenceInterval } from "./ObservationRanges";

export class ObservationFormatter {
    constructor(public ob: FHIR.Observation) {}
    ...
    get codeFull() {
        return (
            (this.ob.code.text ?? (this.ob.code.coding || []).map((c) => c.display ?? c.code).join(", ")) +
            "\n\n" +
    ...
    get interpret(): Interpretation {
        const value = parseFloat(this.value ?? "");

        const reference = lookupReferenceInterval(this.code, this.units);
...
function formatQuantity(val?: FHIR.Quantity): string | null {
    if (!val) {
        return null;
    }

    return `${val.comparator || ""}${val.comparator ? " " : ""}${val.value?.toFixed(1) ?? "???"}`;
}

function formatCodeableConcept(val?: FHIR.CodeableConcept): string | null {
    if (!val) {
        return null;
    }
    if (val.text) {
