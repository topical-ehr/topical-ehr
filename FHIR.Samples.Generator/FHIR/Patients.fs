...
let conditions = ConditionsCsvFormat.Load("conditions.csv").Rows |> Seq.map (fun c -> c.Condition)

let getConditionsFor (patient:Patient) =
    let patientId = patient.Identifier |> Seq.find (fun i -> i.Type.Coding.[0].Code = "MR")
    let rand = new System.Random(Convert.ToInt32(patientId.Value))
    let numConditions = rand.Next(0, 6)
    let conditions = conditions |> Seq.take numConditions |> Seq.toArray
...
let createConditions createResource (patient:Resource) = seq {
...
let createFromBogus (createResource:CreateResource) =
    ...
    let resource = create sample patientId gender prefix firstName lastName middleName medicareNo medicareLineNo
    createResource resource :?> Patient

let createPatientsFromCSV (createResource: CreateResource)  = seq {
    let sample = Bogus.Person()
    let random = Bogus.Randomizer()
    ...
    for pt in patients.Rows do
        ...
        let gender = match pt.Gender with "F" -> Name.Gender.Female | "M" -> Name.Gender.Male | _ -> failwithf "invalid gender field: %s" (pt.Gender)
        let prefix = None
        ...
        let conditions = createConditions createResource created |> Seq.toList
...
let createAdHocPatient (createResource: CreateResource)  =
    let patient =
        Hl7.Fhir.Model.Patient(
            Extension = L [
                Extension(PatExtensions.Urls.ADHOC_PATIENT, FhirBoolean(N true))
