...
let getAnswer answerFraction (qi:Questionnaire.ItemComponent) (patient:Patient) (encounter:Encounter) (performer:Resource) (qr:QuestionnaireResponse) =
    ...
    let datetime = DateTimeOffset(2017, patientDOB.Month,patientDOB.Day, 10, 0, 0, (TimeSpan(10,0,0)))

    let questionId = qi.LinkId
    let questionSummary = qi.Code |> Seq.map (fun c -> c.Display) |> Seq.head
    ...
    let makeObservation _ valueElement = 
        Observation(
            ...
            Comment = if answerFraction > 0.5f then "some comment" else null
        )
    ...
    | "2e708e2-14b9-40bf-9c72-0e757f7e2c41" ->
        // YesNo answer
        makeObservation "Have you ever had a CPAP sleep study" (CodeableConcept(Coding=L[Coding("http://hl7.org/fhir/v2/0136", "Y")]))

    | "611d7b2f-3cce-4f33-8940-d684c1a910d9" ->
        // Month
        makeObservation "When" (FhirDateTime(2017, 03))

    | "4dbabfb0-e1f7-43d8-8393-a2d5555947d0" ->
        // Selection answer
        makeObservation "Mornings with headache per week" (CodeableConcept(Text="1 to 3", Coding=L[Coding("https://metadata.patsoftware.com.au/v3.0/FHIR/codeSystem/uuid/4dbabfb0-e1f7-43d8-8393-a2d5555947d0", "1to3")]))
    ...
    | _ ->
        let shouldAnswer = faker.Random.Float(0.0f, 1.0f) < answerFraction
            ...
            let value : Element =
                ...
                | Some _type ->
                    ...
                    | Questionnaire.QuestionnaireItemType.Integer ->
                        upcast Hl7.Fhir.Model.Quantity(Value= (N 8m))

                    | Questionnaire.QuestionnaireItemType.Date ->
                        upcast FhirDateTime(2018, 6)
                    ...
                    | _ ->
                        failwithf "unhandled question type %A" _type
...
let create (createResource:CreateResource) answerFraction ((patient:Patient),conditions) (nurse:Practitioner) (doctor:Practitioner) (encounter:Encounter) (questionnaire:Questionnaire) =

    let time = N (DateTimeOffset(2017,5,1,10,0,0, (TimeSpan(10,0,0))))

    let section = QuestionSectionType.GetQuestionnaireIdentifierSectionType (questionnaire.Identifier |> Seq.exactlyOne)
    let source : Resource =
        match section with
    ...
    let qr = 
                ...
                if answerFraction = 0.0f then
                    ...
                    [
                        QuestionnaireResponse.ItemComponent(
                            ...
                            Answer = L[ QuestionnaireResponse.AnswerComponent(Value=FhirString("frustrated when can't fall asleep, tired and irritable all day"))]
                        );
                        QuestionnaireResponse.ItemComponent(
                            LinkId = "2e708e2-14b9-40bf-9c72-0e757f7e2c41", // Have you ever had a CPAP sleep study
                            Answer = L[ QuestionnaireResponse.AnswerComponent(Value=Coding("http://hl7.org/fhir/v2/0136", "N"))
        ...
        QuestionnaireResponse(
            ...
            AuthoredElement = FhirDateTime(DateTime(2017, 02, 01, 9,30,0)),
            Item = L items,
            Extension = L[
                            Extension(PatExtensions.Urls.QuestionnaireResponse.SortKey, FhirString("sortkey"))
                            Extension(PatExtensions.Urls.QuestionnaireResponse.AutomatchedReason, ResourceReference(sprintf "Condition/%s" someCondition.Id))
                         ]
        ) |> createResource :?> QuestionnaireResponse

    let observations = seq {
        for item in questionnaire.Item do
            let observation = getAnswer answerFraction item patient encounter performer qr
            yield observation
    }
