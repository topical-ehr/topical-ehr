...
let create createResource (NeedsSubclinicSelection needsSubclinicSelection) (IsFinished isFinished) ((patient: Resource), _) (nurse: Practitioner) =

    let encounter =
        Encounter(
            Subject = referenceToResource patient,
            Status = N (if isFinished then Encounter.EncounterStatus.Finished else Encounter.EncounterStatus.InProgress),
            Extension = L[
                Extension(PatExtensions.Urls.Encounter.NeedsSubclinicSelection, FhirBoolean(N needsSubclinicSelection))
            ...
            Type = L [PatCodes.EncounterTypes.OnSiteClinic; PatCodes.EncounterTypes.OnSiteClinicWithText],
            Period = Period(
                        StartElement = FhirDateTime(2017, 10, 1, 9, 30),
                        EndElement = if isFinished then FhirDateTime(2017, 10, 1, 10, 55) else null
                     ),
            Contained = L [
                Condition(
                    Id = "CurrentCondition1",
                    //Extension = L [ Extension("https://example.com/test-extension", FhirString("test")) ],
            ...
            Participant = L [
                Encounter.ParticipantComponent(
                    Type = L [CodeableConcept("http://hl7.org/fhir/v3/ParticipationType", "ATND", "attender", "treating nurse" )],
                    Individual = ResourceReference(
                        Reference = (sprintf "%s/%s" nurse.TypeName nurse.Id),
                        Display = (nurse.Name.[0].GivenElement.[0].Value) + " " + (nurse.Name.[0].FamilyElement.Value) 
                    )
                )
            ],
            Diagnosis = L[
                Encounter.DiagnosisComponent(Condition=ResourceReference("#CurrentCondition1"))
