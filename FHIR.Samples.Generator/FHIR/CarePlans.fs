...
let makeCarePlans (context:SampleContext) =
    ...
    let time = DateTime.Parse("2017-01-01T14:00:00").ToUniversalTime().ToFhirDateTime()
    ...
    let careplans = seq {
        ...
        yield CarePlan(
            ...
            Category = L [ PatCodes.CarePlan.Category.PatChronicDiseaseManagement; PatCodes.CarePlan.Category.PatChronicDiseaseManagementNoText ],
            ...
            Extension = L[
                Extension(PAT.FHIR.Extensions.PatExtensions.Urls.ActivityTriggeredButDeleted, FhirString(Guid().ToString()))
            ...
            Activity = L[
                CarePlan.ActivityComponent(
                    Detail = CarePlan.DetailComponent(
                        ...
                        Code = CodeableConcept(
                            Coding = L [Coding(KnownUrls.patGoalId, "B5B21335-731C-4F1B-9D82-84EBD19D22CD")],
                ...
                CarePlan.ActivityComponent(
                    Detail = CarePlan.DetailComponent(
                        Code = CodeableConcept(
                            Coding = L [Coding(KnownUrls.patGoalId, "New-B5B21335-731C-4F1B-9D82-84EBD19D22CD")],
                            Text = "'Problem field' goes here for a newly added activity not based on an ActivityDefinition"
                        ),
                        ReasonCode = L [CodeableConcept(Text="'Goal' field goes here")],
                        Description = "Management plan goes here",
                        ...
                        Performer = L[referenceToResource context.careteamForPharmacists],
                        Extension = L[
                            Extension(PAT.FHIR.Extensions.PatExtensions.Urls.ActivityAddedUnder, FhirString("patient-section"))
                            Extension(PAT.FHIR.Extensions.PatExtensions.Urls.CLINICAL_CATEGORY, FhirString("general"))
                        ]
                    ),
                    Progress = L[
                                  Annotation(
                ...
                CarePlan.ActivityComponent(
                    Detail = CarePlan.DetailComponent(
                        Code = CodeableConcept(
                            Coding = L [Coding(KnownUrls.patGoalId, "New-with-bad-UUID-and-no-text")]
                        ),
                        Status = N CarePlan.CarePlanActivityStatus.InProgress,
                        Prohibited = N false,
                        Performer = L[referenceToResource context.careteamForPharmacists],
