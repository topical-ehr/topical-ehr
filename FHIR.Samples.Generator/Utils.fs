...
let createResources (client:FhirClient) (resources:#Resource seq) =
    resources |> Seq.map(createResource client) |> Seq.toList
...
let findAndDelete (fhirClient:FhirClient) (searchCriteria: string array) (resourceType:string) =
    ...
    let mutable found = fhirClient.Search(resourceType, searchCriteria)//, summary = N SummaryType.True
    while found <> null && found.Total.HasValue && found.Total.Value > 0 do
        deleteBundled found
...
let deleteResources (fhirClient:FhirClient) (onlyDeleteSamples: bool) resourceTypesToDelete =
    ...
    resourceTypesToDelete |> List.iter (findAndDelete fhirClient searchCriteria)