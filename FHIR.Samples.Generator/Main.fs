...
let recreatePatients CR fhir onlyDeleteSamples =
    [ "Patient" ] |> Utils.deleteResources fhir onlyDeleteSamples
    let count = 10
    ...
    let patientsFromCSV = FHIR.Patients.createPatientsFromCSV CR |> Seq.toList
...
let main argv = 
    ...
    let fhir = new FhirClient(fhirEndpointUrl, verifyFhirVersion = false (*FIXME*))
    ...
    let CR (resource: #Resource) =
        ...
        created
    let DR (resourceType: string) (criteria: string[]) = Utils.findAndDelete fhir criteria resourceType

    let onlyDeleteSamples = true
    let patientsFromCSV = recreatePatients CR fhir onlyDeleteSamples
    ...
    if haveArg "--deleteSamples" then
        ...
        ["Practitioner"; "PractitionerRole"] |> Utils.deleteResources fhir false
    ...
    let customGoal =
        ...
        let basedOn =
            let bundle = fhir.Search<ActivityDefinition>([| "identifier=urn:uuid:" + bpOutsideRangeGoal |])
            bundle.Entry.[0].Resource :?> ActivityDefinition |> Some
        let useWithClinics =
            let bundle = fhir.Search<PlanDefinition>([| "identifier=" + gpmpClinicGuid |])
            [(bundle.Entry.[0].Resource :?> PlanDefinition)]
        FHIR.ActivityDefinitions.createCustomGoal CR DR {
            name = "Custom Goal 1"
            goal = "custom goal 1"
            plan = "custom plan 1"
            ...
            author = doctor
            priv = false
            dateModified = DateTime(2018, 09, 15)
        } admin |> ignore
    ...
    let sleepCareplan =
        ...
        let encounter = FHIR.Encounter.create CR (NeedsSubclinicSelection true) (IsFinished true) patient nurse

        FHIR.CareTeam.create CR patient "Nurse" "106292003" null |> ignore

        let clinicGuid = "51780bd4-3c9a-446c-86fd-f9cad0aa6501" // sleep clinic
        let clinicIdentifier = sprintf "identifier=%s" clinicGuid
        let questionnaireBundle = fhir.Search<Questionnaire>([| clinicIdentifier |])
        if questionnaireBundle.Entry.Count = 0 then
            printfn "cannot find Questionnaire with %s" clinicIdentifier
        else
            let planDefinitionBundle = fhir.Search<PlanDefinition>([| clinicIdentifier |])
            if planDefinitionBundle.Entry.Count <> 1 then failwithf "cannot find PlanDefinition with %s" clinicIdentifier

            let planDefinition = planDefinitionBundle.Entry.[0].Resource :?> PlanDefinition
            let activityDefinition = (planDefinition.Action |> Seq.find (fun a -> a.TriggerDefinition.Count > 0)).Definition

            for qe in questionnaireBundle.Entry do
                let questionnaire = qe.Resource :?> Questionnaire
                let section = PAT.FHIR.Questions.QuestionSectionType.GetQuestionnaireIdentifierSectionType (questionnaire.Identifier |> Seq.exactlyOne)
            ...
            let createdCareplan = Utils.createResources fhir careplans |> Seq.head
            FHIR.DocumentReference.create CR patient encounter createdCareplan |> ignore
            FHIR.Task.create CR patient encounter createdCareplan |> ignore
            FHIR.Provenance.create CR doctor careplans |> ignore
            FHIR.ReferralRequest.create CR patient doctor pharmacist encounter |> ignore
        
    let gpmpAnswered =
        let patient = patientsFromCSV.[1]
        let encounter = FHIR.Encounter.create CR (NeedsSubclinicSelection false) (IsFinished true) patient nurse

        let clinicGuid = "13885b18-d54c-4ce1-ba4c-4e2509c9fabf" // GPMP
        let clinicIdentifier = sprintf "identifier=%s" clinicGuid
        let questionnaireBundle = fhir.Search<Questionnaire>([| clinicIdentifier |])
        if questionnaireBundle.Entry.Count = 0 then failwithf "cannot find Questionnaire with %s" clinicIdentifier
        ...
        let procedures = FHIR.Procedure.create CR answerAll patient nurse doctor encounter planDefinition
        ...
        FHIR.DocumentReference.create CR patient encounter createdCareplan1 |> ignore
        FHIR.Task.create CR patient encounter createdCareplan1 |> ignore
        FHIR.Provenance.create CR doctor createdCareplans|> ignore
        FHIR.ReferralRequest.create CR patient doctor pharmacist encounter |> ignore
